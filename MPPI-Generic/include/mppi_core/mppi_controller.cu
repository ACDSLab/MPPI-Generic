...
VanillaMPPI::VanillaMPPIController(DYN_T* model, COST_T* cost, float dt, int max_iter, float gamma, int num_timesteps,
...
void VanillaMPPI::computeControl(state_array state) {
    ...
    for (int opt_iter = 0; opt_iter < num_iters_; opt_iter++) {
        ...
        //Generate noise data
        curandGenerateNormal(gen_, control_noise_d_, NUM_ROLLOUTS*num_timesteps_*DYN_T::CONTROL_DIM, 0.0, 1.0);

        //Launch the rollout kernel
        mppi_common::launchRolloutKernel<DYN_T, COST_T, NUM_ROLLOUTS, BDIM_X, BDIM_Y>(model_->model_d_, cost_->cost_d_, dt_, num_timesteps_,
        ...
        HANDLE_ERROR( cudaMemcpyAsync(nominal_control_.data(), nominal_control_d_,
                sizeof(float)*num_timesteps_*DYN_T::CONTROL_DIM, cudaMemcpyDeviceToHost, stream_));
...
void VanillaMPPI::allocateCUDAMemory() {
    ...
    HANDLE_ERROR(cudaMalloc((void**)&nominal_control_d_, sizeof(float)*DYN_T::CONTROL_DIM*num_timesteps_));
    HANDLE_ERROR(cudaMalloc((void**)&nominal_state_d_, sizeof(float)*DYN_T::STATE_DIM*num_timesteps_));
    ...
    HANDLE_ERROR(cudaMalloc((void**)&control_noise_d_, sizeof(float)*DYN_T::CONTROL_DIM*num_timesteps_*NUM_ROLLOUTS));
}
...
template<class DYN_T, class COST_T, int MAX_TIMESTEPS, int NUM_ROLLOUTS, int BDIM_X, int BDIM_Y>
void VanillaMPPI::computeNominalStateTrajectory(
        const VanillaMPPIController::state_array &x0) {
