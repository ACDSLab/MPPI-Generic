...
void launchEnforceConstraintTestKernel( DYNAMICS_T& dynamics, std::vector<std::array<float, S_DIM>>& state,
  ...
  dim3 numBlocks(1, 1);
  enforceConstraintTestKernel<DYNAMICS_T, S_DIM, C_DIM><<<numBlocks,threadsPerBlock>>>(
...
void launchUpdateStateTestKernel( DYNAMICS_T& dynamics, std::vector< std::array<float, S_DIM>>& state,
  ...
  dim3 threadsPerBlock(count, dim_y);
  dim3 numBlocks(1, 1);
  updateStateTestKernel<DYNAMICS_T, S_DIM><<<numBlocks,threadsPerBlock>>>(
...
void launchComputeKinematicsTestKernel( DYNAMICS_T& dynamics, std::vector< std::array<float, S_DIM>>& state,
  ...
  dim3 threadsPerBlock(count, dim_y);
  dim3 numBlocks(1, 1);
  computeKinematicsTestKernel<DYNAMICS_T, S_DIM><<<numBlocks,threadsPerBlock>>>(
          static_cast<DYNAMICS_T*>(dynamics.model_d_), state_d, state_der_d, count);
  CudaCheckError();
...
void launchComputeDynamicsTestKernel( DYNAMICS_T& dynamics, std::vector< std::array<float, S_DIM>>& state,
  ...
  dim3 numBlocks(1, 1);
  // launch kernel
  computeDynamicsTestKernel<DYNAMICS_T, S_DIM, C_DIM><<<numBlocks,threadsPerBlock>>>(dynamics.model_d_, state_d, control_d, state_der_d, count);
  CudaCheckError();
