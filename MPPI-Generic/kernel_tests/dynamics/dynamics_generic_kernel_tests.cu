...
void launchControlRangesTestKernel(DYNAMICS_T& dynamics, std::array<float2, C_DIM>& control_rngs) {
  ...
  controlRangesTestKernel<DYNAMICS_T, PARAMS_T, C_DIM><<<1,1>>>(static_cast<DYNAMICS_T*>(dynamics.model_d_), ranges_d);
  CudaCheckError();
...
void launchEnforceConstraintTestKernel( DYNAMICS_T& dynamics, std::vector<std::array<float, S_DIM>>& state,
  ...
  dim3 numBlocks(1, 1);
  enforceConstraintTestKernel<DYNAMICS_T, PARAMS_T, S_DIM, C_DIM><<<numBlocks,threadsPerBlock>>>(
          static_cast<DYNAMICS_T*>(dynamics.model_d_), state_d, control_d, count);
  CudaCheckError();
...
void launchUpdateStateTestKernel( DYNAMICS_T& dynamics, std::vector< std::array<float, S_DIM>>& state,
  ...
  dim3 threadsPerBlock(count, dim_y);
  dim3 numBlocks(1, 1);
  updateStateTestKernel<DYNAMICS_T, PARAMS_T, S_DIM><<<numBlocks,threadsPerBlock>>>(
          static_cast<DYNAMICS_T*>(dynamics.model_d_), state_d, state_der_d, dt, count);
  CudaCheckError();
...
__global__ void computeStateDerivTestKernel( DYNAMICS_T* dynamics, float* state, float* control, float* state_der, int num) {
  int tid = blockIdx.x*blockDim.x + threadIdx.x;
  if(tid < num) {
    dynamics->computeStateDeriv(state+(tid*S_DIM), control+(tid*C_DIM), state_der+(tid*S_DIM), nullptr);
  }
}
...
void launchComputeStateDerivTestKernel( DYNAMICS_T& dynamics, std::vector< std::array<float, S_DIM>>& state,
  ...
  dim3 threadsPerBlock(count, dim_y);
  dim3 numBlocks(1, 1);
  computeStateDerivTestKernel<DYNAMICS_T, PARAMS_T, S_DIM, C_DIM><<<numBlocks,threadsPerBlock>>>(
