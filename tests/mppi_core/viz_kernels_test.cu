...
protected:
  void SetUp() override {
    ...
    HANDLE_ERROR(cudaMalloc((void**)&initial_state_d,
                            sizeof(float) * CartpoleDynamics::STATE_DIM * 2));
    // create control u trajectory cuda array
    HANDLE_ERROR(cudaMalloc((void**)&control_d,
                            sizeof(float) * CartpoleDynamics::CONTROL_DIM *
                            num_timesteps * 2));
    ...
    HANDLE_ERROR(cudaMalloc((void**)&result_state_d,
                            sizeof(float) * num_rollouts * CartpoleDynamics::STATE_DIM * 2));
    // Create result state cuda array
    HANDLE_ERROR(cudaMalloc((void**)&crash_status_d,
                            sizeof(float) * num_rollouts * 2));
    ...
    HANDLE_ERROR(cudaMemcpyAsync(initial_state_d, x0.data(),
                                 sizeof(float) * CartpoleDynamics::STATE_DIM,
...
TEST_F(VizualizationKernelsTest, stateAndCostTrajectoryKernelNoZNoFeedbackTest) {
  ...
  mppi_common::launchStateAndCostTrajectoryKernel
          <CartpoleDynamics, CartpoleQuadraticCost, DeviceDDP<CartpoleDynamics, 10>, 32, 4>
    (dynamics.model_d_, cost.cost_d_, fb_controller.getDevicePointer(),
     control_d, initial_state_d,
     result_state_d, trajectory_costs_d, crash_status_d,
     num_rollouts, num_timesteps, dt, stream);

  // Copy the costs back to the host
  HANDLE_ERROR(cudaMemcpyAsync(result_state.data(),
                               result_state_d,
                               num_rollouts * CartpoleDynamics::STATE_DIM * sizeof(float),
                               cudaMemcpyDeviceToHost, stream));
  HANDLE_ERROR(cudaMemcpyAsync(trajectory_costs.data(),
                               trajectory_costs_d,
                               num_rollouts * sizeof(float),
                               cudaMemcpyDeviceToHost, stream));
  HANDLE_ERROR(cudaMemcpyAsync(crash_status.data(),
                               crash_status_d,
                               num_rollouts * sizeof(int),
                               cudaMemcpyDeviceToHost, stream));
  HANDLE_ERROR(cudaStreamSynchronize(stream));
}
