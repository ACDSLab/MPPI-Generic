...
#include <kernel_tests/mppi_core/weightedreduction_kernel_test.cuh>
...
TEST(WeightedReductionKernel, strideControlWeightReduction) {
    ...
    for (int i = 0; i < ((num_rollouts-1)/sum_stride + 1); ++i) { // i is the cell index
        for (int j = 0; j < sum_stride; ++j) { // j is the stride index
            for (int k = 0; k < num_timesteps; ++k) { // k specifies the current timestep
