...
using LSTM = LSTMHelper<LSTMParams<8, 20, 0>, FNNHelper<FNNParams<28, 3>>>;
using INIT_LSTM = LSTMHelper<LSTMParams<8, 60, 0>, FNNHelper<FNNParams<68, 100, 40>>>;
...
TEST_F(LSTMLSTMHelperTest, BindStreamAndConstructor)
{
  ...
  EXPECT_NE(helper.lstm_, nullptr);

  std::shared_ptr<INIT_LSTM> init_lstm = helper.getInitModel();
  EXPECT_NE(init_lstm, nullptr);

  // TODO make sure the initial hidden/cell states are zero for init network
...
using T = LSTMLSTMHelper<INIT_LSTM, LSTM, 10>;
TEST_F(LSTMLSTMHelperTest, initializeLSTMTest)
{
  T helper;
...
// using LSTM = LSTMLSTMHelper<LSTMParams<8, 20>, FNNHelper<FNNParams<28, 3>>>;
...
// }
//
// TEST_F(LSTMLSTMHelperTest, UpdateModel)
// {
//   LSTM model;
//
//   int grid_dim = 5;
...
// TEST_F(LSTMLSTMHelperTest, LoadModelNPZTest)
// {
...
//
//   // launch kernel
//   launchParameterCheckTestKernel<LSTMLSTMHelper<LSTMParams<6, 32, 32, 4>>, 1412, 6, 4>(
