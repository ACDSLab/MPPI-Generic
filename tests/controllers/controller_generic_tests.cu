...
public:
  TestController(MockDynamics* model, MockCost* cost, FEEDBACK_T* fb_controller, float dt, int max_iter, float lambda, float alpha,
                 const Eigen::Ref<const control_array>& control_variance,
                 int num_timesteps = 100,
                 const Eigen::Ref<const control_trajectory>& init_control_traj = control_trajectory::Zero(),
                 cudaStream_t stream = nullptr) : Controller<MockDynamics, MockCost, FEEDBACK_T, NUM_TIMESTEPS, number_rollouts, 1, 2>(
                         model, cost, fb_controller, dt, max_iter, lambda, alpha, control_variance, num_timesteps,
...
TEST_F(ControllerTests, ConstructorDestructor) {
  ...
  EXPECT_CALL(*mockDynamics, GPUSetup()).Times(1);
  // EXPECT_CALL(mockFeedback, GPUSetup()).Times(1);
  TestController* controller_test = new TestController(mockDynamics,
    mockCost, mockFeedback, dt, max_iter, lambda, alpha, control_var,
    num_timesteps, init_control_trajectory, stream);
...
TEST_F(ControllerTests, interpolateFeedback) {
  ...
  for(double i = 0; i < controller->getNumTimesteps() - 1; i += 0.25) {
    ...
    TestController::control_array result = controller->interpolateFeedback(state, interpolated_state, i*controller->getDt(), fb_state);
...
TEST_F(ControllerTests, getCurrentControlTest) {
  ...
  for(double i = 0; i < controller->getNumTimesteps() - 1; i += 0.25) {
    ...
    TestController::control_array result = controller->getCurrentControl(state, i*controller->getDt(),
            interpolated_state, traj, fb_state);
