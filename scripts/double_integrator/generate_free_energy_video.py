...
title = None
ln3, = ax.plot([], [], 'r', alpha=0.7, label='Nominal')
ln2, = ax.plot([], [], 'b', alpha=0.7, label='Real')
ln1, = ax.plot(np.linspace(0,80,100), 1*np.ones(100), 'k', alpha=0.7, label='Crash')
...
def main(args):
    ...
    ymax = max(1.1,np.log(max(np.amax(real_fe), np.amax(nominal_fe))))

    # Set up formatting for the movie files
    Writer = animation.writers['ffmpeg']
    writer = Writer(fps=fps, metadata=dict(artist='Manan Gandhi'), bitrate=-1)
