...
void launchAutoRallyWeightedReductionKernelTest(std::array<float, NUM_ROLLOUTS> exp_costs,
  ...
  HANDLE_ERROR(cudaMemcpyAsync(exp_costs_d_, exp_costs.data(), sizeof(float)*exp_costs.size(), cudaMemcpyHostToDevice, stream));
  HANDLE_ERROR(cudaMemcpyAsync(perturbed_controls_d_, perturbed_controls.data(), sizeof(float)*perturbed_controls.size(), cudaMemcpyHostToDevice, stream));
  ...
  autorallyWeightedReductionKernel<CONTROL_DIM, NUM_ROLLOUTS, BLOCKSIZE_WRX>
          ...
          (exp_costs_d_, perturbed_controls_d_, normalizer, NUM_TIMESTEPS);
  CudaCheckError();

  // Copy the result back to the GPU
  HANDLE_ERROR(cudaMemcpyAsync(controls_out.data(), perturbed_controls_d_, sizeof(float)*controls_out.size(), cudaMemcpyDeviceToHost, stream));
...
void launchWeightedReductionKernelTest(std::array<float, NUM_ROLLOUTS> exp_costs,
  ...
  mppi_common::weightedReductionKernel<CONTROL_DIM, NUM_ROLLOUTS, SUM_STRIDE>
          <<<dimGrid, dimBlock, 0, stream>>>
