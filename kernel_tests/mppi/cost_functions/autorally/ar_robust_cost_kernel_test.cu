...
void launchGetCostmapCostTestKernel(CLASS_T& cost, std::vector<std::array<float, 9>>& test_xu, std::vector<float>& cost_results) {
  ...
  dim3 numBlocks(1, 1);
  getCostmapCostTestKernel<CLASS_T><<<numBlocks,threadsPerBlock>>>(static_cast<CLASS_T*>(cost.cost_d_), test_xu_d, cost_results_d, num_test_points);
  CudaCheckError();
...
void launchComputeCostTestKernel(CLASS_T& cost, std::vector<std::array<float, 9>>& test_xu, std::vector<float>& cost_results) {
  ...
  dim3 threadsPerBlock(num_test_points, 1);
  dim3 numBlocks(1, 1);
  computeCostTestKernel<CLASS_T><<<numBlocks,threadsPerBlock>>>(static_cast<CLASS_T*>(cost.cost_d_), test_xu_d, cost_results_d, num_test_points);
  CudaCheckError();
  cudaDeviceSynchronize();
