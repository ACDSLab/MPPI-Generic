...
template<typename COST_T>
__global__ void computeCostTestKernel(COST_T* cost, float* test_xu, float* cost_results, int* timestep, int* crash, int num_points) {
...
void launchComputeCostTestKernel(const COST_T& cost, std::vector<std::array<float, 9>>& test_xu,
  ...
  dim3 threadsPerBlock(num_test_points, 1);
  dim3 numBlocks(1, 1);
  computeCostTestKernel<<<numBlocks,threadsPerBlock>>>(cost.cost_d_, test_xu_d, cost_results_d, timestep_d, crash_d, num_test_points);
  CudaCheckError();
  cudaDeviceSynchronize();
