...
__global__ void textureTestKernel(TEX_T& tex,
                                  DATA_T* test_results, float4* test_indexes, int num_points) {
...
std::vector<DATA_T> getTextureAtPointsKernel(const TEX_T& helper, std::vector<float4>& query_points) {
  ...
  HANDLE_ERROR(cudaMemcpy(tex_query_points_d, query_points.data(),
                          sizeof(float4) * num_test_points, cudaMemcpyHostToDevice));

  dim3 threadsPerBlock(num_test_points, 1);
  dim3 numBlocks(1, 1);
  textureTestKernel<<<numBlocks, threadsPerBlock>>>(*helper.ptr_d_,
                                                    tex_results_d, tex_query_points_d, num_test_points);
  CudaCheckError();
  cudaDeviceSynchronize();

  HANDLE_ERROR(
          cudaMemcpy(results.data(), tex_results_d, sizeof(DATA_T) * num_test_points, cudaMemcpyDeviceToHost));
...
template<class TEX_T, class DATA_T>
__global__ void textureAtMapPoseTestKernel(TEX_T& tex,
                                  DATA_T* test_results, float4* test_indexes, int num_points) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid < num_points) {
    // query texture
...
std::vector<DATA_T> getTextureAtMapPointsKernel(const TEX_T& helper, std::vector<float4>& query_points) {
  ...
  dim3 threadsPerBlock(num_test_points, 1);
  dim3 numBlocks(1, 1);
  textureAtMapPoseTestKernel<<<numBlocks, threadsPerBlock>>>(*helper.ptr_d_,
                                                    tex_results_d, tex_query_points_d, num_test_points);
  CudaCheckError();
  cudaDeviceSynchronize();
...
template<class TEX_T, class DATA_T>
__global__ void textureAtWorldPoseTestKernel(TEX_T& tex,
                                           DATA_T* test_results, float4* test_indexes, int num_points) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid < num_points) {
    // query texture
...
std::vector<DATA_T> getTextureAtWorldPointsKernel(const TEX_T& helper, std::vector<float4>& query_points) {
  ...
  dim3 threadsPerBlock(num_test_points, 1);
  dim3 numBlocks(1, 1);
  textureAtWorldPoseTestKernel<<<numBlocks, threadsPerBlock>>>(*helper.ptr_d_,
                                                             tex_results_d, tex_query_points_d, num_test_points);
  CudaCheckError();
  cudaDeviceSynchronize();
