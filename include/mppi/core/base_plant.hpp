...
public:
  ...
  BasePlant(std::shared_ptr<CONTROLLER_T> controller, int hz, int optimization_stride)
  {
    ...
    controller_->template resizeTimeTrajectory<DYN_T::CONTROL_DIM>(control_traj_);
    controller_->template resizeTimeTrajectory<DYN_T::STATE_DIM>(state_traj_, controller_->getNumTimesteps());
    controller_->template resizeTimeTrajectory<DYN_T::OUTPUT_DIM>(output_traj_, controller_->getNumTimesteps());
    control_traj_.setZero();
    state_traj_.setZero();
    output_traj_.setZero();
    ...
    init_u_ = controller_->model_->getZeroControl();
