...
public:
  ...
  static const int BUFFER_SIZE =
      CONTROLLER_T::TEMPLATED_DYNAMICS::STATE_DIM + CONTROLLER_T::TEMPLATED_DYNAMICS::CONTROL_DIM;
  typedef Eigen::Matrix<float,
                        CONTROLLER_T::TEMPLATED_DYNAMICS::STATE_DIM + CONTROLLER_T::TEMPLATED_DYNAMICS::CONTROL_DIM, 1>
      buffer_state;  //
  typedef Eigen::Matrix<float,
                        CONTROLLER_T::TEMPLATED_DYNAMICS::STATE_DIM + CONTROLLER_T::TEMPLATED_DYNAMICS::CONTROL_DIM,
                        BUFFER_LENGTH + 1>
  ...
  void updateState(StateArray& state, double time)
  {
    ...
    // double temp = this->last_used_pose_update_time_ +
    // this->controller_->getDt()*this->controller_->getNumTimesteps();
    bool t_within_trajectory =
        time >= this->last_used_pose_update_time_ &&
    ...
    auto c_array = this->controller_->getCurrentControl(state, time_since_last_opt, target_nominal_state,
                                                        this->control_traj_, this->feedback_state_);
  ...
  buffer_trajectory getSmoothedBuffer()
  {
    ...
    {
      // printf("at index %d: times size %d, states size %d, knots size %d", j, times.size(), states[j].size(),
      // correct_knots.size()); std::cout << std::endl;
