                                      ...
                                      const float* __restrict__ init_x_d, float* __restrict__ y_d)
{
  ...
  float* x_temp;
  float* xdot = &(reinterpret_cast<float*>(xdot_shared)[shared_idx * DYN_T::STATE_DIM]);
                                           ...
                                           const float* __restrict__ init_x_d, float* __restrict__ y_d)
{
  ...
  const int size_of_theta_d_bytes =
      ...
      sample_dim * distribution_dim * mppi::math::int_ceil(SAMPLING_T::SHARED_MEM_REQUEST_BLK_BYTES, sizeof(float4));
  float* theta_d_shared = &theta_s_shared[size_of_theta_s_bytes / sizeof(float)];
  float* theta_fb_shared = &theta_d_shared[size_of_theta_d_bytes / sizeof(float)];
  ...
  dynamics->initializeDynamics(x, u, y, theta_s_shared, 0.0, dt);
  sampling->initializeDistributions(y, 0.0, dt, theta_d_shared);
  fb_controller->initiliazeFeedback(x, u, theta_fb_shared, 0.0, dt);
  for (int t = 0; t < num_timesteps; t++)
  {
    // Load noise trajectories scaled by the exploration factor
    sampling->readControlSample(global_idx, t, distribution_idx, u, theta_d_shared, blockDim.y, thread_idy, y);
                                       ...
                                       float* __restrict__ trajectory_costs_d)
{
  ...
  extern __shared__ float entire_buffer[];
  float* y_shared = entire_buffer;
  float* u_shared = &y_shared[mppi::math::nearest_quotient_4(num_shared * COST_T::OUTPUT_DIM)];
  float* running_cost_shared = &u_shared[mppi::math::nearest_quotient_4(num_shared * COST_T::CONTROL_DIM)];
  int* crash_status_shared = (int*)&running_cost_shared[mppi::math::nearest_quotient_4(num_shared * 2)];
  float* theta_c = (float*)&crash_status_shared[mppi::math::nearest_quotient_4(num_shared)];
  const int size_of_theta_c_bytes =
      mppi::math::int_ceil(COST_T::SHARED_MEM_REQUEST_GRD_BYTES, sizeof(float4)) +
      num_shared * mppi::math::int_ceil(COST_T::SHARED_MEM_REQUEST_BLK_BYTES, sizeof(float4));
  float* theta_d = &theta_c[size_of_theta_c_bytes / sizeof(float)];

  const int size_of_theta_d_bytes =
      mppi::math::int_ceil(SAMPLING_T::SHARED_MEM_REQUEST_GRD_BYTES, sizeof(float4)) +
      num_shared * mppi::math::int_ceil(SAMPLING_T::SHARED_MEM_REQUEST_BLK_BYTES, sizeof(float4));
                             ...
                             cudaStream_t stream, bool synchronize)
{
  ...
  rolloutCostKernel<DYN_T, COST_T, COST_BLOCK_X><<<dimCostGrid, dimCostBlock, cost_shared_size, stream>>>(
