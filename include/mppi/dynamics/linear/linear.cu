...
void LinearModel::updateState(Eigen::Ref<state_array> state, Eigen::Ref<state_array> state_der, const float dt)
{
  state += state_der * dt;
...
__device__ void LinearModel::computeDynamics(float* state, float* control, float* state_der, float* theta_s)
{
  state_der[0] = this->params_.c_t * control[0] - this->params_.c_b * control[1] - this->params_.c_v * state[0] +
                 this->params_.c_0;
  state_der[1] = (state[0] / this->params_.wheel_base) * tan(control[3]);
