 ...
 **/
#define COLORED_NOISE ColoredNoiseDistribution<DYN_PARAMS_T>

#include <mppi/utils/cuda_math_utils.cuh>
#include <mppi/utils/math_utils.h>

template <class DYN_PARAMS_T>
void COLORED_NOISE::freeCudaMem()
{
  if (GPUMemStatus_)
  {
    // cudaFree(mean_d_);
    // cudaFree(this->control_samples_d_);
...
__host__ void COLORED_NOISE::allocateCUDAMemoryHelper()
{
  if (GPUMemStatus_)
  {
    ...
    const int freq_size = this->params_.num_timesteps / 2 + 1;
    HANDLE_ERROR(cudaMallocAsync((void**)&samples_in_freq_complex_d_,
                                 sizeof(cufftComplex) * this->params_.num_rollouts * CONTROL_DIM * freq_size *
                                     this->params_.num_distributions,
                                 this->stream_));
    HANDLE_ERROR(cudaMallocAsync((void**)&noise_in_time_d,
                                 sizeof(float) * this->params_.num_rollouts * CONTROL_DIM *
                                     this->params_.num_timesteps * this->params_.num_distributions,
                                 this->stream_));
    ...
    // Recreate FFT Plan
    HANDLE_CUFFT_ERROR(cufftPlan1d(&plan_, this->params_.num_timesteps, CUFFT_C2R,
                                   this->params_.num_rollouts * this->params_.num_distributions));
                                                ...
                                                const int block_size, const int thread_index)
{
  ...
  if (CONTROL_DIM % 4 == 0)
  {
    float4* du4 = reinterpret_cast<float4*>(
        &theta_d[sizeof(SAMPLING_PARAMS_T) / sizeof(float) + CONTROL_DIM * threadIdx.x]);  // TODO: replace with theta_d
                                             ...
                                             curandGenerator_t& gen)
{
  ...
  const int num_trajectories = this->params_.num_rollouts * this->params_.num_distributions;

  std::vector<float> sample_freq;
  fftfreq(this->params_.num_timesteps, sample_freq);
  const float cutoff_freq = fmaxf(this->params_.fmin, 1.0 / this->params_.num_timesteps);
  ...
  for (int i = 0; i < CONTROL_DIM; i++)
  {
    ...
    sigma[i] += powf(sample_freqs(freq_size - 1, i) * ((1.0 + (this->params_.num_timesteps % 2)) / 2.0), 2);
    sigma[i] = 2 * sqrt(sigma[i]) / this->params_.num_timesteps;
  }

  // Sample the noise in frequency domain and reutrn to time domain
  const int batch = num_trajectories * CONTROL_DIM;
  // Need 2 * (this->params_.num_timesteps / 2 + 1) * batch of randomly sampled values
  ...
  const int reorder_grid_y = (this->params_.num_timesteps - 1) / BLOCKSIZE_Y + 1;
  ...
  rearrangeNoise<<<reorder_grid, reorder_block, 0, this->stream_>>>(
      noise_in_time_d, control_noise_d_, frequency_sigma_d_, num_trajectories, this->params_.num_timesteps, CONTROL_DIM,
      optimization_stride);

  dim3 control_writing_grid;
  control_writing_grid.x = mppi::math::int_ceil(this->params_.num_rollouts, BLOCKSIZE_X);
  control_writing_grid.y = mppi::math::int_ceil(this->params_.num_timesteps, BLOCKSIZE_Y);
  control_writing_grid.z = mppi::math::int_ceil(this->params_.num_distributions, BLOCKSIZE_Z);
  unsigned int std_dev_mem_size = this->params_.num_distributions * CONTROL_DIM;
  ...
  unsigned int shared_mem_size =
      std_dev_mem_size +
      mppi::math::nearest_quotient_4(this->params_.num_distributions * this->params_.num_timesteps * CONTROL_DIM) +
  ...
  setGaussianControls<<<control_writing_grid, this->params_.readControlsBlockDim, shared_mem_size, this->stream_>>>(
      this->control_mean_d_, this->std_dev_d_, this->control_samples_d_, CONTROL_DIM, this->params_.num_timesteps,
      this->params_.num_rollouts, this->params_.num_distributions, optimization_stride,
