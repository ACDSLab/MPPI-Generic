...
void CONTROLLER::copySampledControlFromDevice()
{
  ...
  {
    HANDLE_ERROR(cudaMemcpyAsync(sampled_controls_[i].data(), control_noise_d_ + samples[i] * control_trajectory_size,
                                 sizeof(float) * control_trajectory_size, cudaMemcpyDeviceToHost, stream_));
  }
  HANDLE_ERROR(cudaStreamSynchronize(stream_));
}
