...
float QuadrotorQuadraticCost::computeStateCost(const Eigen::Ref<const state_array> s, int timestep, int* crash_status) {
  ...
  Eigen::Quaternionf q_g(this->params_.q_goal()[0], this->params_.q_goal()[1],
                         this->params_.q_goal()[2], this->params_.q_goal()[3]);
  ...
  if (this->params_.use_euler) {
    ...
    angle_coeff << this->params_.roll_coeff, this->params_.pitch_coeff,
                   this->params_.yaw_coeff;
    rot_diff = angle_diff;
    ...
    q_coeff << this->params_.q_coeff, this->params_.q_coeff,
               this->params_.q_coeff, this->params_.q_coeff;
    rot_diff = q_vec;
    rot_coeff = q_coeff;
  }
